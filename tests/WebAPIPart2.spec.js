...
let webBrowserContext;
test.beforeAll(async({browser})=>{
    const context = await browser.newContext();
    const page = await context.newPage();
    await page.goto("https://rahulshettyacademy.com/client");
    await page.locator("#userEmail").fill("a.vineel@hotmail.com");
    await page.locator("#userPassword").fill("AAbb11!!");
    await page.locator("[value='Login']").click();
    await page.waitForLoadState('networkidle');
    await context.storageState({path: 'state.json'});
    webBrowserContext = await browser.newContext({storageState:'state.json'});
});

test.only('Browser Context-Validating Error login', async() =>{

    
    var email = "a.vineel@hotmail.com";
    //Use webBrowserContext from beforeAll and inject context to avoid login
    const page = await webBrowserContext.newPage();
    await page.goto("https://rahulshettyacademy.com/client");
